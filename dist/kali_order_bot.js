...
function isAdminPrivateChat(ctx) {
...
function importCSVtoItems(csvPath) {
    if (!fs_1.default.existsSync(csvPath))
        return [];
    const csv = fs_1.default.readFileSync(csvPath, "utf8");
...
function exportItemsToCSV(jsonPath, csvPath) {
    ...
    backupFile(csvPath);
    fs_1.default.writeFileSync(csvPath, csvRows.join("\n"));
}
// --- Order Schedule (Round 1, Round 2) ---
function getOrderRound(now) {
...
function handlePaymentStatus(order, status) {
...
bot.callbackQuery(["select_kitchen", "select_bar"], async (ctx) => {
    ...
    const categories = loadJson(CATEGORY_JSON).filter((c) => c.parent_category === parent);
...
// === ADMIN FLOW ===
// --- Admin Main Menu ---
const adminMenuKeyboard = new grammy_1.InlineKeyboard()
...
const adminFileMenu = new grammy_1.InlineKeyboard()
...
function getFileActionsKeyboard(key) {
    const actions = new grammy_1.InlineKeyboard()
...
bot.on('message:document', async (ctx) => {
    ...
    for (const file of files) {
        ...
        if (file.txt && fname === file.txt) {
            ...
            backupFile(txtPath);
            fs_1.default.writeFileSync(txtPath, txt);
...
// --- Edit Files Menu ---
const adminFilesKeyboard = new grammy_1.InlineKeyboard()
...
bot.callbackQuery("edit_csv_items", async (ctx) => {
    const csv = fs_1.default.existsSync(ITEM_CSV) ? fs_1.default.readFileSync(ITEM_CSV, "utf8") : "CSV file not found.";
    await ctx.reply(`CSV Items (edit and send back):\n\n${csv}`);
});
bot.callbackQuery("edit_json_items", async (ctx) => {
    const json = fs_1.default.existsSync(ITEM_JSON) ? fs_1.default.readFileSync(ITEM_JSON, "utf8") : "[]";
    await ctx.reply(`JSON Items (edit and send back):\n\`\`\`json\n${json}\n\`\`\``);
});
bot.callbackQuery("edit_json_categories", async (ctx) => {
    const json = fs_1.default.existsSync(CATEGORY_JSON) ? fs_1.default.readFileSync(CATEGORY_JSON, "utf8") : "[]";
    await ctx.reply(`JSON Categories (edit and send back):\n\`\`\`json\n${json}\n\`\`\``);
});
bot.callbackQuery("edit_json_suppliers", async (ctx) => {
    const json = fs_1.default.existsSync(SUPPLIER_JSON) ? fs_1.default.readFileSync(SUPPLIER_JSON, "utf8") : "[]";
...
bot.on('message:text', async (ctx, next) => {
    ...
    if (text.startsWith("CSV Items")) {
        ...
        backupFile(ITEM_CSV);
        fs_1.default.writeFileSync(ITEM_CSV, csvData);
    ...
    if (text.startsWith("{") || text.startsWith("[")) {
        try {
            ...
            if (json.item_sku && !Array.isArray(json)) {
                ...
                const idx = itemsArr.findIndex((i) => i.item_sku == json.item_sku);
                if (idx >= 0) {
                    itemsArr[idx] = json;
                    backupFile(ITEM_JSON);
...
bot.callbackQuery(/admin_supplier_order:(.+)/, async (ctx) => {
    ...
    const itemsArr = loadJson(ITEM_JSON).filter((i) => i.supplier == supplierId);
    const menu = new grammy_1.InlineKeyboard();
    itemsArr.forEach((item) => menu.text(item.item_name, `admin_item_action_menu:${item.item_sku}`).row());
...
bot.callbackQuery(/admin_item_action_menu:(.+)/, async (ctx) => {
    ...
    const submenu = new grammy_1.InlineKeyboard()
...
bot.callbackQuery(/admin_item_action:(setqty|remove|assign):(.+)/, async (ctx) => {
    ...
    let itemsArr = loadJson(ITEM_JSON);
    const idx = itemsArr.findIndex((i) => String(i.item_sku) === itemSku);
    if (idx < 0)
    ...
    if (action === "setqty") {
        ...
        const suppliers = loadJson(SUPPLIER_JSON).filter((s) => s.enabled);
        const menu = new grammy_1.InlineKeyboard()
            .text("Kali", `admin_assign:kali:${itemSku}`).row()
            .text("Alternative", `admin_assign:alt:${itemSku}`).row();
        suppliers.forEach((s) => menu.text(s.supplier, `admin_assign:${s.supplier}:${itemSku}`).row());
...
bot.command('manager_processing', async (ctx) => {
    ...
    await ctx.reply('Manager processing orders:', {
        reply_markup: new grammy_1.InlineKeyboard().text('Complete', 'complete_order').row()
    });
});
...
// --- Edit Layouts ---
const layoutsMenu = new grammy_1.InlineKeyboard()
