...
function backupFile(path) {
    if (fs_1.default.existsSync(path)) {
        fs_1.default.copyFileSync(path, `${path}.bak_${Date.now()}`);
    }
}
// === BOT INSTANCE ===
...
const pendingApprovals = {}; // messageId -> item details
const pendingDispatch = {}; // messageId -> item details
const pendingPolls = {}; // pollId -> poll details
// Mark mode tracking
const userMarkMode = {}; // userId -> isMarkMode
const markedItems = {}; // userId -> { itemSku: item }
const userMarkContext = {}; // userId -> current context (kitchen/bar/categories)
// === USER FLOW ===
const userContext = {};
let kitchenOrder = {};
let barOrder = {};
function threadLink(chatId, topicId) {
...
function buildMarkModeKeyboard(userId, context) {
...
// --- Start Command ---
const startReplyKeyboard = new grammy_1.Keyboard()
...
bot.hears(["Kitchen", "Bar"], async (ctx) => {
    ...
    if (userMarkMode[userId]) {
        ...
        const subCats = subCategories[parent] || [];
        const replyKeyboard = new grammy_1.Keyboard();
        ...
        for (let i = 0; i < subCats.length; i += 3) {
            ...
            if (row[1])
                replyKeyboard.text(row[1]);
            if (row[2])
                replyKeyboard.text(row[2]);
            replyKeyboard.row();
        }
...
bot.hears("Place Order", async (ctx) => {
    ...
    const destinationKeyboard = new grammy_1.InlineKeyboard()
...
bot.hears("Today List", async (ctx) => {
    ...
    if (fs_1.default.existsSync(todayListPath)) {
        const todayList = fs_1.default.readFileSync(todayListPath, 'utf8');
...
bot.hears("Custom List", async (ctx) => {
    ...
    if (fs_1.default.existsSync(customListPath)) {
        const customList = fs_1.default.readFileSync(customListPath, 'utf8');
...
bot.hears(["meat", "fish/seafood", "dairy", "veggies", "spices", "dry", "sauce", "cleaning", "plastics",
    ...
    if (!subCategory)
        return;
    const items = loadJson(ITEM_JSON).filter((i) => i.sub_category === subCategory);
    if (items.length === 0) {
    ...
    const itemsKeyboard = new grammy_1.InlineKeyboard();
    if (userMarkMode[userId]) {
        ...
        items.forEach((item) => itemsKeyboard.text(item.item_name, `add_to_order:${item.item_sku}`).row());
    }
...
bot.callbackQuery(/mark_item:(.+)/, async (ctx) => {
    ...
    const item = items.find((i) => String(i.item_sku) === itemId);
    if (!item)
        return await ctx.answerCallbackQuery("Item not found");
    ...
    const allItems = loadJson(ITEM_JSON).filter((i) => i.sub_category === subCategory);
    const itemsKeyboard = new grammy_1.InlineKeyboard();
...
bot.callbackQuery(/show_items:(\d+)/, async (ctx) => {
    ...
    const items = loadJson(ITEM_JSON).filter((i) => i.category_id == categoryId);
    const itemsKeyboard = new grammy_1.InlineKeyboard();
    items.forEach((item) => itemsKeyboard.text(item.item_name, `add_to_order:${item.item_sku}`).row());
...
bot.callbackQuery(/go_back_to_categories:(\d+)/, async (ctx) => {
    ...
    const parentCategories = categories.filter((c) => c.parent_category === parent);
    const catsKeyboard = new grammy_1.InlineKeyboard();
    parentCategories.forEach((cat) => catsKeyboard.text(cat.category_name, `show_items:${cat.category_id}`).row());
    await ctx.editMessageText(`Categories for ${parent.charAt(0).toUpperCase() + parent.slice(1)}:`, { reply_markup: catsKeyboard });
    if (ctx.from)
        userContext[ctx.from.id] = "categories";
});
// === V2 CORE ORDER FLOW ===
...
bot.callbackQuery(/add_to_order:(.+)/, async (ctx) => {
    ...
    const defaultQty = item.default_quantity || '1';
    const approvalKeyboard = new grammy_1.InlineKeyboard()
        .text("+1", `qty_add:${item.item_sku}:${defaultQty}`)
        .text("✅", `approve_item:${item.item_sku}:${defaultQty}`)
        .text("❌", `cancel_item:${item.item_sku}`);
    const approvalMessage = await bot.api.sendMessage(GROUP_CHAT_ID, `${item.item_name} ${defaultQty}`, {
...
bot.callbackQuery(/qty_add:(.+):(.+)/, async (ctx) => {
    ...
    const updatedKeyboard = new grammy_1.InlineKeyboard()
...
bot.callbackQuery(/dispatch_approve:(.+):(.+)/, async (ctx) => {
    ...
    await ctx.editMessageText(`✅ DISPATCHED: <<${dispatch.supplier}>>\n${item.item_name} ${dispatch.quantity} ${item.measure_unit}\n•\n\n${dispatch.dateStamp}`, { reply_markup: undefined });
    ...
    const pollMessage = await bot.api.sendPoll(GROUP_CHAT_ID, `Confirm receipt of items from ${dispatch.supplier} - ${dispatch.dateStamp}?`, [`${item.item_name} (${dispatch.quantity} ${item.measure_unit})`], {
...
bot.callbackQuery(/dispatch_reject:(.+):(.+)/, async (ctx) => {
    ...
    await ctx.editMessageText(`❌ DISPATCH REJECTED: <<${dispatch.supplier}>>\n${item.item_name} ${dispatch.quantity} ${item.measure_unit}\n•\n\n${dispatch.dateStamp}`, { reply_markup: undefined });
    // Clean up dispatch tracking
    delete pendingDispatch[messageId];
...
bot.on('poll_answer', async (ctx) => {
    ...
    if (ctx.pollAnswer.option_ids.length > 0) {
        ...
        const completedKeyboard = new grammy_1.InlineKeyboard()
            .text("📊 CRM", `crm_update:${pollId}`);
        await bot.api.sendMessage(GROUP_CHAT_ID, `🎉 COMPLETED!\n\n<<${poll.supplier}>>\n${poll.item.item_name} ${poll.quantity} ${poll.item.measure_unit}\n•\n\n${poll.dateStamp}`, {
...
bot.hears(/Kitch Order \(\d+\)/, async (ctx) => {
    ...
    await ctx.reply("Tap below to open Kitchen Topic:", {
        reply_markup: new grammy_1.InlineKeyboard().url("Go to Kitchen Topic", url)
    });
});
bot.hears(/Bar Order \(\d+\)/, async (ctx) => {
    ...
    await ctx.reply("Tap below to open Bar Topic:", {
        reply_markup: new grammy_1.InlineKeyboard().url("Go to Bar Topic", url)
    });
});
...
bot.on(['message:text', 'message:photo', 'message:voice'], async (ctx, next) => {
    if (ctx.message.reply_to_message && ctx.message.reply_to_message.text && ctx.message.reply_to_message.text.includes("custom item request")) {
        ...
        await bot.api.sendMessage(GROUP_CHAT_ID, `📋 Custom Item Approval Required from ${ctx.from?.username || ctx.from?.first_name || 'Unknown'}`, {
            message_thread_id: MANAGER_TOPIC_ID,
            reply_markup: approvalKeyboard
        });
...
bot.hears("Go Back", async (ctx) => {
    ...
    if (userId && userContext[userId] === "items") {
        const categories = loadJson(CATEGORY_JSON).filter((c) => c.parent_category === "kitchen");
        const catsKeyboard = new grammy_1.InlineKeyboard();
        categories.forEach((cat) => catsKeyboard.text(cat.category_name, `show_items:${cat.category_id}`).row());
...
bot.hears("Search", async (ctx) => {
    await ctx.reply("Type @kalisystembot <item> in any chat to search instantly, or tap below to start inline search.", {
        reply_markup: new grammy_1.InlineKeyboard().switchInlineCurrent("").row()
    });
});
...
bot.inlineQuery(/.*/, async (ctx) => {
    ...
    if (query.startsWith('1 1')) {
        ...
        filteredItems = items.filter((item) => item.category_id < 30000 && foodSubCategories.includes(item.sub_category));
    }
    // Kitchen Households (1 2) 
    else if (query.startsWith('1 2')) {
        const householdSubCategories = ['cleaning', 'plastics', 'dry'];
        filteredItems = items.filter((item) => item.category_id < 30000 && householdSubCategories.includes(item.sub_category));
    }
    // Bar Food (2 1)
    ...
    // Fallback: search all items by name if query doesn't match pattern
        ...
        filteredItems = items.filter((item) => item.item_name.toLowerCase().includes(searchQuery));
    }
    // Generate item results with "Add to order" buttons
    const results = filteredItems.slice(0, 50).map((item) => ({
        type: "article",
        id: item.item_sku,
